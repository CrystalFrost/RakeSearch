			...
			// Подбор значения для очередной клетки квадрата
				...
				// Генерируем новое значение для клетки (rowId, columnId)
					...
					// Подбираем значение для клетки
						// Так как для 9 ранга в рамках маски workunit-а уже заданы обе диагонали, то проверку по ним - отключаем
...
{
	const unsigned int rowsHistoryMasks[Rank] = { 510, 509, 507, 503, 495, 479, 447, 383, 255 };	// Битовые флаги истории задействования строк в рамках текущей комбинации, где i-е значение - это маска
	int currentSquareRows[Rank] = { 0, -1, -1, -1, -1, -1, -1, -1, -1 };					// Массив с перечнем текущих строк, использованных в квадрате. На i-й позиции - номер используемой строки
	unsigned int primaryDiagonalFlags = 510;												// Битовые флаги свободных значений на основной диагонали 
	unsigned int secondaryDiagonalFlags = 255;												// Битовые флаги свободных значений на побочной диагонали 
	unsigned int rowsUsageFlags = 510;														// Битовые флаги задействования строк в текущей комбинации
	unsigned int rowsHistoryFlags[Rank] = { 510, 509, 507, 503, 495, 479, 447, 383, 255 };	// Битовые флаги истории задействования строк в рамках текущей комбинации, где i-е значение - это маска
	...
	// Важная деталь! В состав битовых флагов истории зайдействования строк, уже включён фильтр, отсеивающий комбинацию, приводящую к исходному квадрату
