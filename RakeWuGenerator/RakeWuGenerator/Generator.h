...
private:
	...
	/*static const int CellsInPath = 18;		// Число обрабатываемых клеток для маленького WU. Для диагоналей 9 ранга + 3 клетка в первой строке - 18 клеток*/
	...
	void GatherStatistics();				// Подсчёт статистических показателей (дублирует механизмы ProcessWorkunit*!)

	int diagonalCounter;					// Для статистики. Значение счётчика заданий при переходе на новую диагональ
	int diagonalValue;						// Для статистики. Значение элементов главной диагонали при переходе на её новое значение
	int diagonalStartNumber;				// Для статичтики. Номер задания с которого начинается диагональ
